...
public class Deck : MonoBehaviour
{
    ...
    {
        //создать точку привязки для всех игровых объектов xard в иерархии
    ...
    public void ReadDeck(string deckXMLText)
    {
        ...
        for (int i = 0; i < xCardDefs.Count; i++)
        {
            ...
            cDef.rank = int.Parse(xCardDefs[i].att("rank"), CultureInfo.InvariantCulture);
    ...
    private Card MakeCard(int cNum)
    {
        ...
        //настроить transform.parent новой карты в соответсвтии с точкой привязуки
        cgo.transform.parent = deckAnchor;
        Card card = cgo.GetComponent<Card>();

        //Выкладывает карьыт в аккуратный ряд
    ...
    private void AddDecorators(Card card)
    {
        foreach(Decorator deco in decorators)
        {
            ...
            _tSR.sortingOrder = 1; //поместить справйты над картой
            _tGO.transform.SetParent(card.transform); //сделать справйт дочерним по отношению к карте
            ...
            _tGO.name = deco.type; //дать объекту имя
            card.decoGOs.Add(_tGO); //добавть объект в список
        }
    }
}
